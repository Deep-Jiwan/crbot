...
ANNOTATED_FOLDER = os.getenv("ANNOTATED_FOLDER", "frame")
...
print(f"Subscriber started. Receiving frames from {ZMQ_ADDRESS} and publishing win/lose on port {PUB_PORT}")
print(f"Annotation set to: {ANNOTATE}")
print(f"Checking 4 points:")
print(f"  p1({WINNER_X1}, {WINNER_Y1}), p2({WINNER_X2}, {WINNER_Y1})")
print(f"  p3({WINNER_X1}, {WINNER_Y2}), p4({WINNER_X2}, {WINNER_Y2})")
print(f"Target1 (p1,p2): BGR({TARGET_B1}, {TARGET_G1}, {TARGET_R1}) tolerance {TOLERANCE1}")
print(f"Target2 (p3,p4): BGR({TARGET_B2}, {TARGET_G2}, {TARGET_R2}) tolerance {TOLERANCE2}")
...
def process_frame(img):
    ...
    if p12_both_match and not p34_both_match:
        ...
        if ANNOTATE:
            print("No clear result - entering 3 second backoff")
            print(f"p1&p2 match target1: {p12_both_match}, p3&p4 match target2: {p34_both_match}")
        return None
    ...
    if ANNOTATE:
        ...
        print(f"p1({WINNER_X1},{WINNER_Y1}): BGR({b1},{g1},{r1}) - Match T1: {p1_match}")
        print(f"p2({WINNER_X2},{WINNER_Y1}): BGR({b2},{g2},{r2}) - Match T1: {p2_match}")
        print(f"p3({WINNER_X1},{WINNER_Y2}): BGR({b3},{g3},{r3}) - Match T2: {p3_match}")
        print(f"p4({WINNER_X2},{WINNER_Y2}): BGR({b4},{g4},{r4}) - Match T2: {p4_match}")
